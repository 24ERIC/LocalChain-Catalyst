...
function SubmitProposal() {
  ...
  useEffect(() => {
    ...
    const isFormFilled = isNameValid && isCategoryValid && isDescriptionValid && isCheckboxChecked;
    setIsFormValid(isFormFilled);
  }, [proposalName, proposalCategory, proposalDescription, droppedFile, isCheckboxChecked]);
  ...
  return (
      <div>
        ...
        <div style={{ margin: "40px 0" }}>
          ...
          />
          <div>
            <Text sx={{ color: '#C1C2C5', fontWeight: 500 }}>Enter Image URL:</Text>
            <input
                type="text"
                value={imageUrl}
                onChange={handleUrlChange}
                style={{ width: '600px', maxWidth: '100%', marginBottom: '40px' }}
            />

            {imageUrl && (
                <>
                  <Text sx={{ color: '#C1C2C5', fontWeight: 500 }}>Image preview:</Text>
                  <Image
                      src={imageUrl}
